...
/*Function to read parameters of each core returns 0 if reading value successfully*/
    ...
    while (fgets(line, sizeof(line), fp)) {
            if (strncmp(line, "cpu", 3) == 0) {
                ...
                if (line[3] == ' ') {
                    ...
                    sscanf(line, "cpu  %llu %llu %llu %llu %llu %llu %llu",&cores[0].user, &cores[0].nice, &cores[0].system, &cores[0].idle, &cores[0].iowait, &cores[0].irq,&cores[0].softirq);
                
                } else if (isdigit(line[3])) {
                    int id;
                    sscanf(line, "cpu%d %llu %llu %llu %llu %llu %llu %llu",&id,&cores[count].user, &cores[count].nice, &cores[count].system, &cores[count].idle, &cores[count].iowait, &cores[count].irq,&cores[count].softirq);
                }
            count++;
        }
...
/*Calculate total CPU parameters*/
...
/*Calculate CPU parameters in idle mode*/
...
static void get_cpu_usages(void) {
    ...
    unsigned long long t1, t2 ,i1 ,i2;

    if (read_core_stats(cores_before, &count_1) == -1) {
        printf("read_core_stats(before) failed\n");
        return;
    }
    sleep(TIME_CALCULATE);
    if (read_core_stats(cores_after, &count_2) == -1) {
        printf("read_core_stats(after) failed\n");
        return;
    }

    n = (count_1 < count_2) ? count_1 : count_2;

    for (int i = 0; i < n; i++) {
        t1 = total_target(cores_before[i]);
        t2 = total_target(cores_after[i]);
        i1 = idle_target(cores_before[i]);
        i2 = idle_target(cores_after[i]);
...
/*Calculate top 5 CPU-using applications*/
